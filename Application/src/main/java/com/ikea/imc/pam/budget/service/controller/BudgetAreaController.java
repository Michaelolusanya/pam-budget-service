...
public class BudgetAreaController {
    ...
    public ResponseEntity<ResponseMessageDTO<BudgetAreaDTO>> getBudgetArea(@PathVariable Long id) {
        ...
        return budgetAreaService
                .getBudgetArea(id)
                .map(
                        budgetArea -> ResponseEntityFactory.generateResponse(
                                HttpStatus.OK, budgetAreaMapper.buildBudgetAreaDTO(budgetArea)))
                .orElseGet(
                        () -> {
                            log.debug("Could not find budget area with id {}", id);
                            return ResponseEntityFactory.generateResponseMessage(
                                    HttpStatus.NOT_FOUND, String.format("BudgetArea %d not found", id));
                        }
                );
    }
    ...
    public ResponseEntity<ResponseMessageDTO<BudgetAreaDTO>> findBudgetArea(
            @RequestParam(Paths.PARAMETER_PARENT_TYPE) BudgetParentType
            parentType,
            @RequestParam(Paths.PARAMETER_PARENT_ID) @Min(Constants.MINIMUM_ID)
            Long parentId,
            @RequestParam(Paths.PARAMETER_FISCAL_YEAR) @Min(Constants.MINIMUM_YEAR) @Max(Constants.MAXIMUM_YEAR)
            Integer fiscalYear) {

        BudgetAreaParameters budgetAreaParameters = new BudgetAreaParameters(parentType, parentId, fiscalYear);

        return budgetAreaService
                .findBudgetArea(budgetAreaParameters)
                .map(
                        budgetArea -> ResponseEntityFactory.generateResponse(
                                HttpStatus.OK, budgetAreaMapper.buildBudgetAreaDTO(budgetArea)))
                .orElseGet(
                        () -> {
                            ...
                            return ResponseEntityFactory.generateResponseMessage(
                                    ...
                                    String.format("BudgetArea for parentType %s and parentId %d for fiscalYear %d not found",
                                            parentType,
                                            parentId,
                                            fiscalYear));
                        }
    ...
    @PutMapping
    public ResponseEntity<ResponseMessageDTO<BudgetAreaDTO>> putBudgetArea(@RequestBody @Valid BudgetAreaDTO budgetAreaDTO) {
